...
def update(guitar):
    sql = "UPDATE guitars SET (guitar_name,colour,manufacturer_id,guitar_type,no_of_strings,production_date,stock_price,selling_price,quantity) = (%s,%s,%s,%s,%s,%s,%s,%s,%s) WHERE id = %s RETURNING *"
    values = [guitar.guitar_name,guitar.colour,guitar.manufacturer.company_name,guitar.guitar_type,guitar.no_of_strings,guitar.production_date,guitar.stock_price,guitar.selling_price,guitar.quantity,guitar.id]
    print(values)
    run_sql(sql,values)
